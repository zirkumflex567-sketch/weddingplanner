  ...
  useEffect(() => {
    ...
    async function loadSession() {
      ...
      try {
        ...
        if (!cancelled) {
          setError("Dieser RSVP-Link ist nicht mehr gültig oder konnte nicht geladen werden.");
          setStatus("ready");
        }
      }
    }
      ...
      <section className="portal-card">
        <div className="portal-head">
          <div>
            ...
            <h1>
              {session
                ? `${session.context.coupleName} freut sich auf eure Rückmeldung`
                : "RSVP wird geladen"}
            </h1>
          </div>
          <span className={`source-pill source-pill--${status}`}>
            {status === "loading" ? "Lädt" : status === "saving" ? "Speichert" : "Bereit"}
          </span>
        </div>
        ...
        {session ? (
          <>
            <p className="portal-copy">
              {session.guest.name}, ihr seid eingeladen für {session.context.targetDate} in{" "}
              ...
              <div className="portal-section">
                ...
                <label className="portal-field">
                  ...
                  <textarea
                    ...
                    placeholder="Zum Beispiel vegetarisch, keine Nüsse, Kindersitz nötig..."
                  />
                </label>
        ...
        <p className="portal-footnote">
          Wedding Copilot läuft hier als privacy-first Prototyp. KI-Arbeit bleibt auf Shadow,
